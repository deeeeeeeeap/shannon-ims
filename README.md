...
程序会从绝对配置路径推导 `/opt/shannon-ims` 运行根，并将数据库、日志、运营商覆盖和 session secret 固定在该根目录下；因此不会随调用者的当前目录漂移。卸载同样只允许删除该运行根的严格子路径，并会解析现存父目录的物理路径；root 内通过 symlink/junction 重定向到外部的目标会 fail-closed。
...
- 紧急呼叫、商业语音质量或生产级高可用；
- OpenWrt 安装包；仓库中旧的未验证 init/config 片段已移除，当前状态见 [packaging/openwrt/README.md](packaging/openwrt/README.md)；
- 在没有合法授权的情况下绕过运营商策略。
...
restricted to strict children of the same root. Existing parents are resolved
physically, and a target redirected outside the root through a symlink or
...
No supported OpenWrt package is shipped. The former unverified init/config fragments were removed; see [packaging/openwrt/README.md](packaging/openwrt/README.md) before considering a manual port.
