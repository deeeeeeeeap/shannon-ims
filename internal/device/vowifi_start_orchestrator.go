...
func (p *Pool) prepareVoWiFiStartContext(deviceID, traceID, runtimeEPDGOverride string) (voWiFiStartContext, error) {
	...
	if regOpts := carrier.ResolveIMSRegisterProfile(startProfile.MCC, startProfile.MNC); strings.TrimSpace(regOpts.Profile.ContactFeatures) != "" {
		...
		logger.Info("VoWiFi 已启用手机 REGISTER 伪装画像",
			...
			"sip_instance_source", "carrier_preset",
			"register_expires", int(regOpts.RegisterExpiry.Seconds()))
	}
	// Vodafone UK (and other non-preset PLMNs) still need a stable handset-like
	// +sip.instance. Prefer live device IMEI as urn:gsma:imei:... without logging it.
	if strings.TrimSpace(startCtx.SIPInstanceURN) == "" {
		imei := strings.TrimSpace(currentStatus.IMEI)
		if urn := voiceclient.FormatGSMAIMEIURN(imei); urn != "" {
			startCtx.SIPInstanceURN = urn
			logger.Info("VoWiFi 已用设备 IMEI 构建 Contact +sip.instance",
				"trace_id", traceID,
				"device", deviceID,
				"sip_instance_source", "device_imei",
				"sip_instance_kind", "gsma_imei")
		}
	}
