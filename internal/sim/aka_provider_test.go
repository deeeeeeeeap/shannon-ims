...
func TestATAKAProviderUSIMPreservesSyncFailureAUTSWithoutRetry(t *testing.T) {
	...
	modem := &akaProviderModemFake{
		resolvedAID:      "A0000000871002FF44FF128900000100",
		resolveSource:    "test",
		logicalResponses: []string{hex.EncodeToString(response)},
	}
	provider := NewATAKAProvider(modem)

	result, err := provider.CalculateAKAWithPreference(
		bytes16(0x10),
		bytes16(0x20),
		AKAAppPreferenceUSIM,
	)
	...
	if len(modem.logicalCalls) != 1 {
		t.Fatalf("AUTH APDU calls = %d, want 1", len(modem.logicalCalls))
	}
}
